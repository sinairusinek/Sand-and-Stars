...
declare function mapping:SnSEnglish($root as element(), $userParams as map(*)) {
    let $prefix:= 'En'
    let $unique:=substring-after($root/@xml:id,'_')
    let $id:= ``[`{$prefix}`_`{$unique}`]``
    let $node := root($root)/id($id)
    return
        $node
};

declare function mapping:SnSGerman($root as element(), $userParams as map(*)) {
    let $prefix:= 'De'
    let $unique:=substring-after($root/@xml:id,'_')
    let $id:= ``[`{$prefix}`_`{$unique}`]``
    let $node := root($root)/id($id)
...
declare function mapping:SnSHebrew($root as element(), $userParams as map(*)) {
    let $prefix:= 'Heb'
    let $unique:=substring-after($root/@xml:id,'_')
    let $id:= ``[`{$prefix}`_`{$unique}`]``
    let $node := root($root)/id($id)
...
declare function mapping:SnSNotes($root as element(), $userParams as map(*)) {
    let $notes := root($root)//noteGrp
    return
        $notes
};
